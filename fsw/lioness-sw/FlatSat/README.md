...
## Building and Running the FlatSat Application

In order to build the FlatSat application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the FlatSat application's code.
```
fprime-util build
```
...
To run the ground system without starting the FlatSat app:
```
...
```
cd FlatSat/build-artifacts/<platform>/bin/
./FlatSat -a 127.0.0.1 -p 50000
```
